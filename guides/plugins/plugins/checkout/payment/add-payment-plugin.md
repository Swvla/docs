...
All methods - bar the `captureRecurring` - get the `\Shopware\Core\System\SalesChannel\SalesChannelContext` injected. Please note, that this class contains properties, which are nullable. If you want to use this information, you have to ensure in your code that they are set and not `NULL`.
...
class ExamplePayment implements SynchronousPaymentHandlerInterface
{
    ...
    public function captureRecurring(RecurringPaymentTransactionStruct $transaction, Context $context): void
    {
        ...
        try {
            ...
            throw PaymentException::recurring($transaction->getOrderTransaction()->getId(), 'Something went wrong', $e);
        }
    }
}
```
